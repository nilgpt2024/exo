...
class ChatGPTAPI:
  def __init__(
    ...
    cors.add(self.app.router.add_post("/models/{model_name}/unload", self.handle_unload_model), {"*": cors_options})
  ...
  async def handle_unload_model(self, request):
    model_id = request.match_info.get('model_name')
    try:
      if hasattr(self.node.inference_engine, 'unload_model'):
        success = await self.node.inference_engine.unload_model(model_id)
        if success:
          return web.json_response({"status": "success", "message": f"Model {model_id} unloaded from memory"})
        else:
          return web.json_response({"detail": f"Model {model_id} is not loaded in memory"}, status=404)
      else:
        return web.json_response({"detail": "Unload model not supported by current inference engine"}, status=501)
    except Exception as e:
      if DEBUG >= 2: traceback.print_exc()
      return web.json_response({"detail": f"Error unloading model: {str(e)}"}, status=500)
